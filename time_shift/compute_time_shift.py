...
def eval_all(answer_true, answer_pred, onset_tolerance=0.05, print_result=True, output_matched_notes=False):

    avg = np.zeros(15)
    matched_notes_list = []

    best_onset, f1_score, best_shift = find_allshift(answer_true, answer_pred, onset_tolerance=onset_tolerance)
    time_shift = (best_shift[0] + best_shift[1]) / 2

    for i in range(len(answer_true)):
        if output_matched_notes:
            ret, matched_notes = eval_one_data(answer_true[i], answer_pred[i], onset_tolerance=onset_tolerance
                , shifting=time_shift, gt_pitch_shift=0.0, output_matched_notes=output_matched_notes)
        ...
        avg[14] = sub_opt_shift

    if print_result:
        print ("best global time shift:", avg[14])
        print ("Model performance after applying global time shift:")
        ...
        print ("Number of songs:", len(answer_true))
...
def main(args):
    ...
    print (time.time())
    my_eval.accuracy(onset_tolerance=float(args.onset_tol))
    print (time.time())
